#!/usr/bin/env bash
# =============================================================================
# script_nbody.sh -- HWSW project, benchmark #1: nbody
#
#   0. environment setup (perf, python3-dbg, pyperformance, FlameGraph)
#   1. correctness check  (original vs optimized produce the same physics)
#   2. baseline run       (pyperformance, original nbody)
#   3. optimized run      (pyperformance, custom manifest -> bm_nbody_opt)
...
#   9. bytecode count     (executed bytecodes per step, original vs optimized)
#
# Usage:   ./script_nbody.sh            # normal mode
#          MODE=--rigorous ./script_nbody.sh
#          SKIP_SETUP=1 ./script_nbody.sh
...
cp "$NB/optimized/run_benchmark.py" "$NB/manifest/bm_nbody_opt/run_benchmark.py"

# ------------------------------------------------------- 1. correctness -----
log "Correctness check (original vs optimized)"
$PY "$NB/verify_nbody.py" 20000 | tee "$RES/verify.txt"

# ---------------------------------------------------------- 2. baseline -----
log "Baseline: pyperformance run -b nbody $MODE"
rm -f "$RES/baseline.json"            # pyperformance refuses to overwrite
pyperformance run -b nbody $MODE -o "$RES/baseline.json"

# --------------------------------------------------------- 3. optimized ----
log "Optimized: pyperformance run --manifest (nbody_opt) $MODE"
rm -f "$RES/optimized.json"
pyperformance run --manifest "$NB/manifest/MANIFEST" -b nbody_opt $MODE -o "$RES/optimized.json"
...
for v in original optimized; do
  perf stat -r 5 -e "$EVENTS" -- $PY "$NB/profile_nbody.py" "$v" 5 20000 \
...
# profile_nbody.py (same benchmark code, single process, no pyperformance).
...
for v in original optimized; do
  if [[ $v == original ]]; then
    PP_ARGS=(run --bench nbody)
  else
    PP_ARGS=(run --manifest "$NB/manifest/MANIFEST" --bench nbody_opt)
  fi
  ...
  if ! perf record -e "$PERF_EVENT" -F 999 -g -o "$RES/perf_$v.data" -- \
    ...
    echo "!! pyperformance under $PYDBG failed -> fallback: profile_nbody.py"
    perf record -e "$PERF_EVENT" -F 999 --call-graph "$CALLGRAPH" -o "$RES/perf_$v.data" \
      -- $PYDBG "$NB/profile_nbody.py" "$v" 2 20000
  fi
  ...
  if [[ -s "$RES/perf_$v.folded" ]]; then
    "$TOOLS/FlameGraph/flamegraph.pl" --title "nbody ($v) - CPython internals" \
        "$RES/perf_$v.folded" > "$FG/flamegraph_nbody_$v.svg"
  else
...
# ---------------------------------------------------------- 7. ablation -----
log "Ablation study (each optimization separately)"
rm -f "$RES/ablation.json"
$PY "$NB/ablation_nbody.py" $MODE -o "$RES/ablation.json"
$PY - "$RES/ablation.json" <<'EOF' | tee "$RES/ablation.txt"
import sys, pyperf
suite = pyperf.BenchmarkSuite.load(sys.argv[1])
rows = [(b.get_name(), b.mean() * (10 if "numpy" in b.get_name() else 1)) for b in suite.get_benchmarks()]
base = rows[0][1]
print(f"{'variant':34s} {'time per 20k steps':>20s} {'vs original':>12s}")
for name, t in rows:
...
# -------------------------------------------------- 9. bytecode count -----
log "Executed bytecodes per step (original vs optimized)"
$PY "$NB/bytecode_count_nbody.py" > "$RES/bytecode_count.txt" && cat "$RES/bytecode_count.txt"
